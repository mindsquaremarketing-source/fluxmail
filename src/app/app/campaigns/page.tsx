...
export default function CampaignsPage() {
  ...
  const { state, refreshCampaigns } = useApp()
  const { campaigns, loaded } = state
  ...
  const handleSend = async (campaign: any) => {
    ...
    if (!confirm(`Send "${campaign.name}"?`)) return
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/app/campaigns/create">
          <button className="flex items-center gap-2 bg-gray-900 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-gray-800 transition-all">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3l14 9-14 9V3z" /></svg>
            Create Campaign
          </button>
        </Link>
      </div>
                ...
                <tr>
                  <td colSpan={9} className="text-center py-16">
                    <div className="w-16 h-16 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-3">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                    </div>
                    ...
                    <Link href="/app/campaigns/create"><button className="px-4 py-2 bg-blue-700 text-white text-sm rounded-lg hover:bg-blue-800">Create Campaign</button></Link>
                  </td>
                </tr>
                    ...
                    {renamingId === campaign.id ? (
                      <div className="flex items-center gap-2">
                        <input value={renameValue} onChange={e => setRenameValue(e.target.value)}
                          onKeyDown={e => { if (e.key === 'Enter') handleRename(campaign.id); if (e.key === 'Escape') setRenamingId(null) }}
                          autoFocus className="border border-blue-400 rounded-lg px-2 py-1 text-sm w-40" />
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0">
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                        </div>
                        <span className="text-sm font-medium text-gray-900 group-hover:text-blue-700 transition-colors">{campaign.name}</span>
                      </div>
                    )}
                  </td>
                  <td className="px-4 py-4 text-sm text-gray-500 whitespace-nowrap">{new Date(campaign.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</td>
                        ...
                        {openMenuId === campaign.id && (
                          <>
                            ...
                            <div className={`absolute right-0 bg-white border border-gray-200 rounded-xl shadow-xl z-[999] w-44 py-1 ${rowIndex >= filtered.length - 2 ? 'bottom-8' : 'top-8'}`}>
                              <button onClick={() => { setOpenMenuId(null); router.push(`/app/campaigns/${campaign.id}`) }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
                                View Report
                              </button>
                              <button onClick={() => handleDuplicate(campaign.id)} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50">
                              ...
                              <button onClick={() => { setOpenMenuId(null); setRenamingId(campaign.id); setRenameValue(campaign.name) }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50">
                              ...
                              <button onClick={() => handleDelete(campaign.id)} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50">
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden flex flex-col" style={{ maxHeight: '90vh' }}>
              ...
              <div className="flex items-center gap-3">
                {previewCampaign.status !== 'sent' && (
                  <button onClick={() => { setPreviewCampaign(null); handleSend(previewCampaign) }}
                    className="flex items-center gap-2 bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-blue-800">Send Now</button>
                )}
            ...
            <div className="flex-1 overflow-auto bg-gray-100 p-4">
              <iframe srcDoc={previewCampaign.htmlContent || '<p style="text-align:center;padding:40px;color:#999">No preview available</p>'}
                className="w-full bg-white rounded-xl shadow-sm" style={{ height: '500px', border: 'none' }} title="Preview" />
            </div>
          </div>
        </div>
      )}
